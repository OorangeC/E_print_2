...
a.	根据业务员搜索 FindOrdersBySales(string)==>IOrder[]
i.	前端输入：IOrder: sales（string:业务员姓名，例如张三)
ii.	后端返回：IOrder[] (订单数组: 满足IOrder.sales=="张三")
b.	根据审核员搜索 FindOrdersByAudit(string input)==>IOrder[]
i.	前端输入：IOrder.audit（string:审核员姓名，例如李四)
ii.	后端返回：IOrder[] (订单数组: 满足IOrder.audit=="李四")
c.	根据唯一索引搜索唯一订单 FindOrderByID(string input)==>IOrder
     ——判别条件：IOrder.order_unique==input
d.	所有待审核订单 FindPendingOrders(OrderStatus input)==>IOrder
...
a.	根据制单员搜索数组 FindWorkOrdersByClerk(string input)==>IWorkOrder[]  
——判别条件：IWorkOrder.work_clerk==input
b.	根据审核员搜索数组 FindWorkOrdersByAudit(string input)==>IWorkOrder[]
——判别条件：IWorkOrder.work_audit==input
c.	根据工程单号搜索唯一工程单 FindWorkOrderByID(string input)==>IWorkOrder
——判别条件：IWorkOrder.work_unique==input
d.	所有待审核工程单 FindPendingWorkOrders()==>IWorkOrder[]
——判别条件：IWorkOrder:orderstatus==WorkOrderStatus.PENDING_REVIEW
后端
实现上述加粗的四个函数。github已经事实更新 https://github.com/xxxhhhyxy/E_Print
